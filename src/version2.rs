...
pub enum ParseError {
    ...
    #[error("invalid length specified: {}, causes overflow", given)]
    LengthOverflow { given: usize },

    #[error("invalid TLV type id specified: {}", type_id)]
    InvalidTlvTypeId { type_id: u8 },

    #[error("invalid UTF-8: {:?}", bytes)]
    InvalidUtf8 { bytes: Vec<u8> },

    #[error("invalid ASCII: {:?}", bytes)]
    InvalidAscii { bytes: Vec<u8> },

    #[error("trailing data: {:?}", len)]
...
mod parse_tests {
    ...
    fn test_invalid_data() {
        ...
        assert_eq!(parse_fully(&mut &[0][..]), Err(UnexpectedEof));
